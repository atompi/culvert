...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/culvert.yaml)")
}
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".culvert" (without extension).
		viper.AddConfigPath("./")
